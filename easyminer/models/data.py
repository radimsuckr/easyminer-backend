...
class DataSource(Base):
    ...
    upload_id: Mapped[Optional[int]] = mapped_column(ForeignKey("upload.id"), nullable=True)
...
class Field(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True, index=True)
    name: Mapped[str] = mapped_column(String(255))
    data_type: Mapped[str] = mapped_column(String(50))  # e.g., 'string', 'integer', 'float', etc.
    ...
    unique_values_count: Mapped[Optional[int]] = mapped_column(nullable=True)
    missing_values_count: Mapped[Optional[int]] = mapped_column(nullable=True)
    ...
    std_value: Mapped[Optional[float]] = mapped_column(nullable=True)
